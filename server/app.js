...
var pg = require('pg');
var connectionString = 'postgres://localhost:5432/animalchallenge';
...
//inserts animal entered from dom, via client.js into DB 'animals'
app.post('/postAnimals', urlencodedParser, function(req, res){
  console.log('Animal is sent to DB');
  pg.connect(connectionString, function(err, client, done){
    client.query( 'INSERT INTO animals (animal, animalcount) VALUES ($1, $2)', [req.body.animal, number(1, 100)]);
    done();
  });
  res.end();
});
...
app.get('/getAnimal', function(req, res){
  ...
  var results = [];
  pg.connect(connectionString, function(err, client, done){
  var animalRequest=client.query('SELECT * FROM animals ORDER BY id DESC;');
